...
skip_if_live_network_disabled() {
  if [[ -z "${STRAPS_RUN_NETWORK_TESTS:-}" ]]; then
    skip "Live network tests disabled (set STRAPS_RUN_NETWORK_TESTS=1 to enable)"
  fi
}

skip_if_missing_nc() {
  command -v nc >/dev/null 2>&1 || skip "nc command not available"
}
