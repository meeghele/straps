...
@test "can_connect_to with local SSH" {
  ...
  can_connect_to "127.0.0.1" 22 tcp || skip "Local SSH service unavailable"
...
@test "can_connect_to with invalid hostnames" {
  if [[ -n "${STRAPS_RUN_NETWORK_TESTS:-}" ]]; then
    skip_if_no_network
...
@test "dns_resolves handles ipv4 and ipv6" {
  run dns_resolves "localhost"
  [ "$status" -eq 0 ]
}
